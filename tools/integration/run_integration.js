...
(async () => {
    ...
    function spawnClient(name, actionScript) {
        ...
        // The client will assert that it receives at least one `server_status` message
        const waitForServerStatusMs = process.env.WAIT_FOR_SERVER_STATUS_MS || 5000;
        const code = `const clientSim = require(${clientSimLiteral});(async()=>{console.log('[client-${name}] connecting to ${WS_URL}');const ws = await clientSim.connect('${WS_URL}','${ROOM}','${name}');let serverStatusReceived=false;ws.on('message',m=>{try{console.log('[client-${name}] msg:',m.toString());const p=JSON.parse(m.toString());if(p && p.type==='server_status'){serverStatusReceived=true;console.log('[client-${name}] server_status received',p.level)}}catch(e){console.log('[client-${name}] non-json msg')}});ws.send(clientSim.makeTypingMsg(true));await new Promise(r=>setTimeout(r,400));ws.send(clientSim.makeTypingMsg(false));await new Promise(r=>setTimeout(r,400));ws.send(clientSim.makeMessageMsg('hello from ${name}'));if(${actionScript}){await new Promise(r=>setTimeout(r,1200));console.log('[client-${name}] performing forced close');ws.terminate()}await new Promise(r=>setTimeout(r,2000));if(!serverStatusReceived){console.error('[client-${name}] ERROR: no server_status message received within ${waitForServerStatusMs}ms');process.exit(3)}console.log('[client-${name}] exiting normally');process.exit(0)})().catch(e=>{console.error(e);process.exit(2)});`;
